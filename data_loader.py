...
class JointProcessor:
    ...
    def _create_examples(self, texts, intents, slots, set_type):
        ...
        for i, (text, intent, slot) in enumerate(zip(texts, intents, slots)):
            ...
            intent_label = (
                self.intent_label.index(intent) if intent in self.intent_labels else self.intent_labels.index("UNK")
            )
            ...
            examples.append(InputExample(guid, words, intent_label, slot_labels))
        return examples
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, slot_label in zip(examples.words, examples.slot_labels):
            ...
            if not word_tokens:
                word_tokens = [ukn_token]
            tokens.extend(word_tokens)
            slot_labels_ids.extend([int(slot_label)] + [pad_token_id] * (len(word_tokens) - 1))
        ...
        slot_label_ids = [pad_token_label_id] + slot_labels_ids
        ...
        attention_mask = attention_mask + ([1 if mask_padding_with_zero else 0] * padding_length)
        token_type_ids = token_type_ids + ([pad_token_segment_id] * padding_length)
        slot_label_ids = slot_label_ids + ([pad_token_label_id] * padding_length)
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            mode, list(filter(None, args.model_name_or_path.split("/"))).pop(), args.max_seq_len
        ),
    )
