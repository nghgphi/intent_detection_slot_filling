...
class Trainer:
    def __init__(self, args, train_dataset, test_dataset, dev_dataset):
        self.args = args
        self.train_dataset = train_dataset
        self.test_dataset = test_dataset
        self.dev_dataset = dev_dataset
        
        self.intent_label_list = get_intent_labels(args)
        self.slot_label_list = get_slot_labels(args)
        ...
        if args.pretrained:
            ...
            self.model = self.model_class.from_pretrained(
                ...
                slot_labels = self.slot_label_list
            )
        else:
    ...
    def evaluate(self, mode):
        ...
        if slot_preds is None:
            ...
            if self.args.use_crf:
                slot_preds = np.append(slot_preds, self.model.crf.decode(slot_logits), axis=0)
            else:
                slot_preds = np.append(slot_preds, slot_logits.detach().cpu().numpy(), axis=0)
            out_slot_label_ids = np.append(
                out_slot_label_ids, inputs["slot_label_ids"].detach().cpu().numpy(), axis=0
            )
        ...
        for i in range(out_slot_label_ids.shape[0]):
            for j in range(out_slot_label_ids.shape[1]):
                ...
                slot_preds_list[i].append(slot_label_map[slot_preds])
